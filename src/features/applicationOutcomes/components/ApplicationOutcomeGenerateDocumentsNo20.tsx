...
export function ApplicationOutcomeGenerateDocumentsNo20({
  applicationOutcomes,
  categoryId,
}: Props) {
  ...
  const calculateFinalScore2 = (outcome: ApplicationOutcome) => {
    const averageScore = Number(outcome.average_score) || 0; // Garante que seja um número
    const hasBonus10Percent = outcome.application?.data?.bonus?.some((bonus) =>
      bonus.includes("10%: Estudantes que tenham cursado integralmente o ensino médio em escolas públicas.")
    );

    if (hasBonus10Percent) {
      return (averageScore * 1.10).toFixed(2); // Aplica 10% de bônus
    }

    return averageScore.toFixed(2); // Retorna a nota média sem bônus
  };
  ...
  const duplicateEntries = Object.entries(duplicateScores)
    ...
    .sort((a: ApplicationOutcome, b: ApplicationOutcome) => {
      // Usa a "Nota Final 2" para ordenar
      const finalScore2A = Number(calculateFinalScore2(a));
      const finalScore2B = Number(calculateFinalScore2(b));

      if (finalScore2B !== finalScore2A) {
        return finalScore2B - finalScore2A;
      }
      ...
      if (ageB !== ageA) {
        return ageA - ageB; // Mais velho tem prioridade
      }
  ...
  const getFormattedBonus = (bonuses?: string[]) => {
    if (
      ...
      bonuses.includes("20%: Estudantes que tenham cursado e concluído integralmente o ensino médio em instituições de ensino, públicas ou privadas, localizadas na região do Maciço do Baturité.") ||
      bonuses.length === 0
    )
    {
      return "-";
  ...
  const generatePDF = () => {
    ...
    // Modifica as linhas para exibir "Nota Final 2" no lugar da "Nota Final"
          ...
          >
            <thead>
              <tr style={{ border: "1px solid black" }}>
                <th style={{ border: "1px solid black", padding: "8px", whiteSpace: "normal" }}>
                  Classificação
                </th>
                <th style={{ border: "1px solid black", padding: "8px", whiteSpace: "normal" }}>
                  Nome
                ...
                <th style={{ border: "1px solid black", padding: "8px", whiteSpace: "normal" }}>
                  Nota Final 2 (Sem Bonificação dos 20%)
                </th>
                <th style={{ border: "1px solid black", padding: "8px", whiteSpace: "normal" }}>
                  Bonificação
                </th>
            ...
            <tbody>
              {outcomesByCategory.map((outcome, index) => (
                <tr key={outcome.id} style={{ border: "1px solid black", fontWeight: outcome.classification === "Classificado" ? "bold" : "normal" }}>
                  <td style={{ border: "1px solid black", padding: "8px", whiteSpace: "normal" }}>
                    {index + 1}
                  </td>
                  <td style={{ border: "1px solid black", padding: "8px", whiteSpace: "normal" }}>
